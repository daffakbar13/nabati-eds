...
import {
  deleteShippingCondition,
  getListShippingCondition,
  updateStatusShippingCondition,
...
export default function PageTransporationShippingCondition() {
  ...
  const handleChangeStatus = async () => {
    try {
      const res = updateStatusShippingCondition({
  ...
  return (
    <>
      <Text variant={'h4'}>Shipping Condition</Text>
      <Spacer size={20} />
          ...
          <FloatAction>
            <RowAntd justify="space-between" style={{ flexGrow: 1 }}>
              <b style={{ lineHeight: '48px' }}>
                {table.state.selected.length} Shipping Condition are Selected
              </b>
              <RowAntd gutter={10}>
                <ColAntd>
                  <Button
