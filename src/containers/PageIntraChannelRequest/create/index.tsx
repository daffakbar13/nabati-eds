...
export default function PageCreateQuotation() {
  ...
  const onChangeBranch = (form: string, ch: any, branchid: any, label: string) => {
    const sloc = allSloc.filter(
      ({ doc_type_id, branch_id }) => branch_id == branchid && doc_type_id == 'ZINC',
    )

    if (form == 'from') {
      setSupplyingBranch(label)
      getBranchLocation({ id: branchid }).then((result) => {
        setReceivingBranch(`${result.data.branch_to_id} - ${result.data.branch_to_name}`)
        const slocTo = allSloc.filter(
  ...
  React.useEffect(() => {
    getSloc({ id: 'PP01' }).then((result) => setAllScloc(result.data))
  }, [])
      ...
      <Card style={{ overflow: 'unset', padding: '28px 20px' }}>
        <div style={{ display: 'flex', gap: 20 }}>
          <div style={{ display: 'flex', gap: 15, flexDirection: 'column', flexGrow: 1 }}>
            ...
            />
            <Input type="text" label="From Channel" disabled value={fromCH} />
            <Input type="text" label="From Sloc" disabled value={fromsloc} />
            <DebounceSelect
              type="input"
              label="Header Text"
          ...
          <div style={{ display: 'flex', gap: 15, flexDirection: 'column', flexGrow: 1 }}>
            ...
            />
            <Input type="text" label="To Channel" disabled value={toCH} />
            <Input type="text" label="To Sloc" disabled value={toSloc} />
            <DatePickerInput
              fullWidth
              onChange={(val: any) => {
