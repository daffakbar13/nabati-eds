  ...
  return (
    <Modal open={showModal !== undefined} closable={false} footer={footer} zIndex={500}>
      <Typography.Title level={3}>
        {showModal === 'create' ? 'Create' : 'View'} Salesman Division
      </Typography.Title>
      <Row gutter={[10, 10]}>
        <Col span={24}>
          <DebounceSelect type="select" required label="Salesman ID" placeholder={'Select'} />
        </Col>
        <Col span={24}>
          <DebounceSelect type="select" required label="Division ID" placeholder={'Select'} />
        </Col>
        <Col span={24}>
