...
interface QuotationDetail {
  ...
  readonly message: string
  readonly data: {
    readonly result: QuotationDetailResult
    readonly pagination: API.Pagination
  }
}

namespace QuotationApi {
  export function getQuotation(): Promise<QuotationList> {
    return axios
      .get<QuotationList>('http://localhost:3000/data/quotation.json')
      .then((res) => res.data)
  }

  export function getDetailQuotation(id: string): Promise<QuotationDetail> {
    const url = `${API.URL}v1/quotations/${id}`
